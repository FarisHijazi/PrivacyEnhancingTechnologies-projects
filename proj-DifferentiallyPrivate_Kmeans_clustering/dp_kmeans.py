...
def kmeans(X, n_clusters=3, eps=None, STOP_THRESHOLD=0.01, MAX_LOOPS=500,
           seed=None, plot=True, verbose=False, losses=[], plot_every=None, print_every=None):
    """
...
#                                   seed, plot, verbose, losses, plot_every, print_every)
...
#                 noise = get_noise(eps_k_t_, seed=seed)
                ...
                np.random.seed(seed)
                noise = np.random.laplace(loc=np.mean(X_normalized))

                # adding different noises will cause it to diverge

                sum = sum + noise
                num = num + noise  # updating centroids
        ...
        if plot_every is not None and (t % plot_every == 0):
            print('plotting kmeans matrix... (in loop)')
            fig, axes = kmeans_matrix(X, n_clusters, centroids, labels)
            fig.show()

        if verbose or (print_every is not None and (t % print_every == 0)):
        ...
        print(f'Stop condition reached: current error < STOP_THRESHOLD: {STOP_THRESHOLD}, stopping iterations')
