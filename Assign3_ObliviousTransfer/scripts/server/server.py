    ...
    while True:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            with conn:
                ...
                #    generates two random  values, r_0 and r_1, and sends them to Bob along with her PK
                import secrets, json
                ...
                combined_list = [messages[i] + k_list[i] for i in range(args.n_msgs)]
                combined_list = json.dumps(list(map(_bytes_to_string, combined_list))) # serialized
                send_msg(conn, combined_list)
